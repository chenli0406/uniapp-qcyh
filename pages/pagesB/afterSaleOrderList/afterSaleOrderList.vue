<template>
	<view class="container">
		<cu-custom bgColor="nav-bg" :isBack="true" :isShow="true">
			<block slot="backText">售后订单</block>
		</cu-custom>
		...
		<view class="content">
			...
			<view class="cu-list sm-border menu bg-white" v-for="(item, index) in list" :key="index">
				<view style="text-align: right;font-weight: bold;">
					<text v-if="item.status == '1'" style="color:#F8A61F;">待审批</text>
					<text v-if="item.status == '2'" style="color:#EB5757;">未通过</text>
					<text v-if="item.status == '3'" style="color:#29BF54;">已退回</text>
				</view>
